...
every file in the cybergraph has a type. currently type is external metadata — stored alongside the hash, not inside it. this means two files with identical data but different types produce the same particle. type confusion is possible: the same bytes interpreted as "image" and "executable" share a particle.
