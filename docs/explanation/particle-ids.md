...
[[cyber]] is a different system. it has one hash function (Hemera), one field (Goldilocks), one output size (32 bytes), one encoding (little-endian canonical), and one mode (sponge). there is nothing to negotiate, nothing to disambiguate, nothing to version. the system is the description.

| | IPFS CIDv1 | nox particle address |
|---|---|---|
| structure | version + codec + hash-code + length + digest | digest only |
| size | 36-38 bytes typical | 32 bytes fixed |
...
```
inbound:   [multicodec | multihash | digest] → strip → 32-byte address
outbound:  32-byte address → prepend [multicodec | multihash] → CIDv1
```
