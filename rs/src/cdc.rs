...
pub fn chunk_ranges(data: &[u8], element_size: usize) -> Result<ChunkRanges<'_>, CdcError> {
    if !(1..=64).contains(&element_size) {
        return Err(CdcError::ElementSize);
    }
    if !data.len().is_multiple_of(element_size) {
        return Err(CdcError::MisalignedLength);
    }
    Ok(ChunkRanges {
        data,
        element_size,
...
pub fn section_cdc_tree(data: &[u8], element_size: usize, is_root: bool) -> Hash {
    debug_assert!(
        (1..=64).contains(&element_size),
        "element_size {element_size} out of [1, 64]"
    );
    debug_assert!(
        data.len().is_multiple_of(element_size),
        "data length {} not a multiple of element_size {element_size}",
        data.len()
    );
...
mod tests {
    ...
    fn window_size_always_power_of_two() {
        for s in 1..=64usize {
            ...
            assert!(
                w.is_power_of_two(),
                "W={w} not power of two for element_size={s}"
            );
